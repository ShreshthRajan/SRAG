...
def run_phase3_training():
    ...
    if os.path.exists("results/phase3_grpo"):
        ...
        os.makedirs("/results/phase3_grpo_4player", exist_ok=True)

        for item in os.listdir("results/phase3_grpo"):
            src = os.path.join("results/phase3_grpo", item)
            dst = os.path.join("/results/phase3_grpo_4player", item)
            if os.path.isfile(src):
                shutil.copy2(src, dst)
                print(f"  Saved: {item}", flush=True)

        # Save checkpoints (FIXED: copy directories with shutil.copytree)
        checkpoint_dir = "checkpoints/phase3_grpo_4player"
        if os.path.exists(checkpoint_dir):
            for item in os.listdir(checkpoint_dir):
                ...
                if os.path.isfile(src):
                    ...
                    # CRITICAL FIX: Copy directories (like grpo_iteration_N/) with copytree
                    shutil.copytree(src, dst, dirs_exist_ok=True)
                    print(f"  Saved checkpoint dir: {item}/", flush=True)
