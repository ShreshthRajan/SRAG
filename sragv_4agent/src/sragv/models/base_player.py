...
class BasePlayer(ABC):
    ...
    def load_model(self):
        ...
        # CRITICAL: Save reference to base model BEFORE any PEFT wrapping
        # This allows clean adapter loading without double-wrapping issues
        self._base_model_unwrapped = self.model
    ...
    def load_adapter(self, adapter_path: str):
        ...
        CRITICAL FIX: Uses unwrapped base model to avoid double-wrapping bug.
        If load_model() created a fresh adapter, this replaces it with checkpoint weights.
        """
        ...
        # CRITICAL: Use unwrapped base model to prevent double-wrapping
        # This fixes PEFT Issue #2115: "Ineffective Fine-Tuning Bug"
        base_model_for_loading = getattr(self, '_base_model_unwrapped', self.model)

        # Load checkpoint adapter (replaces any fresh adapter from initialization)
        self.peft_model = PeftModel.from_pretrained(base_model_for_loading, adapter_path)
        logger.info(f"Loaded LoRA adapter from {adapter_path}")

    def unload_adapter(self):
        """
        Unload PEFT adapter to use pure base model.
        Critical for baseline evaluation where no adapter should be applied.
        """
        if self.peft_model:
            del self.peft_model
            self.peft_model = None
            torch.cuda.empty_cache()
            logger.info("Unloaded PEFT adapter - using pure base model")
