...
class SelfPlayConfig:
    ...
    # Resume control
    skip_volume_resume: bool = False  # Skip checking volume for checkpoints (useful for verification runs)
...
class SelfPlayTrainer:
    ...
    def _create_grpo_config(self) -> GRPOConfig:
        ...
        return GRPOConfig(
            group_size=self.config.solutions_per_problem,  # Use SelfPlayConfig value for memory control
    ...
    def _attempt_resume_from_volume(self) -> Optional[int]:
        """Check if checkpoint exists in volume and prepare for resume."""
        import shutil

        volume_checkpoint_dir = Path("/results/phase3_grpo")

        # Look for latest checkpoint in volume
        latest_checkpoint = None
        latest_iteration = 0

        if volume_checkpoint_dir.exists():
            for item in volume_checkpoint_dir.glob("checkpoint_iteration_*.json"):
                try:
                    iter_num = int(item.stem.split('_')[-1])
        ...
        if latest_checkpoint and latest_iteration > 0:
            logger.info(f"Found checkpoint in volume at iteration {latest_iteration}")
            logger.info("Copying checkpoint from volume to local filesystem...")

            # Copy checkpoint files from volume to local
            try:
                # Copy main checkpoint
                local_checkpoint_path = self.checkpoint_dir / f"checkpoint_iteration_{latest_iteration}.json"
                shutil.copy2(latest_checkpoint, local_checkpoint_path)

                # Copy GRPO checkpoint directory
                volume_grpo_dir = volume_checkpoint_dir / f"grpo_iteration_{latest_iteration}"
                local_grpo_dir = self.checkpoint_dir / f"grpo_iteration_{latest_iteration}"
                if volume_grpo_dir.exists():
                    shutil.copytree(volume_grpo_dir, local_grpo_dir, dirs_exist_ok=True)

                # Copy MAP-Elites archive
                volume_archive = volume_checkpoint_dir / f"archive_iteration_{latest_iteration}.json"
                local_archive = self.checkpoint_dir / f"archive_iteration_{latest_iteration}.json"
                if volume_archive.exists():
                    shutil.copy2(volume_archive, local_archive)

                logger.info(f"✅ Checkpoint files copied from volume")
                return latest_iteration

            except Exception as e:
                logger.error(f"Failed to copy checkpoint from volume: {e}")
                return None

        return None

    def _load_checkpoint_state(self, iteration: int):
        """Load checkpoint state to resume training."""
        ...
        # Restore convergence metrics
        self.convergence_metrics = checkpoint_data.get("convergence_metrics", {})

        # Restore behavior tracker (approximate - rebuild from summary)
        behavior_summary = checkpoint_data.get("behavior_summary", {})
        # Note: Full behavior history not preserved, but summary is

        # Load GRPO checkpoint
        grpo_checkpoint_dir = self.checkpoint_dir / f"grpo_iteration_{iteration}"
        if grpo_checkpoint_dir.exists():
            self.grpo_trainer.load_checkpoint(str(grpo_checkpoint_dir))
            logger.info(f"✅ GRPO checkpoint loaded")

        # Load MAP-Elites archive
        archive_path = self.checkpoint_dir / f"archive_iteration_{iteration}.json"
        if archive_path.exists():
            self.map_elites_archive.load_archive(str(archive_path))
            logger.info(f"✅ MAP-Elites archive loaded")

        logger.info(f"✅ All checkpoint state restored from iteration {iteration}")
    ...
    def run_self_play_training(self) -> Dict[str, Any]:
        ...
        """
        # Attempt to resume from volume checkpoint (unless skip_volume_resume is set)
        start_iteration = None
        if not self.config.skip_volume_resume:
            start_iteration = self._attempt_resume_from_volume()
        ...
        if start_iteration and start_iteration > 0:
            ...
            # Then load checkpoint state
            self._load_checkpoint_state(start_iteration)
        else:
        ...
        # Load bootstrap data (skip if resuming past bootstrap)
        if not start_iteration or start_iteration < self.config.bootstrap_iterations:
            bootstrap_problems = self._prepare_bootstrap_data()
        else:
            bootstrap_problems = []
            logger.info("Skipping bootstrap data (resuming past bootstrap phase)")
        ...
        try:
            # Run training iterations (start from checkpoint + 1)
            iteration_start = (start_iteration + 1) if start_iteration else 1
            for iteration in range(iteration_start, self.config.num_iterations + 1):
    ...
    def _prepare_bootstrap_data(self) -> List[Dict[str, Any]]:
        ...
        # Select problems for bootstrap phase (use problems_per_iteration, not multiplied)
        # Bootstrap iterations will reuse the same problems to establish baseline
        bootstrap_problems = problems[:self.config.problems_per_iteration]
    ...
    def _run_single_iteration(
        ...
        try:
            ...
            # Add verification generation training data (Player 3)
            for i, test_case in enumerate(all_test_cases[:self.config.problems_per_iteration]):  # Limit for memory
                problem_id = test_case.get('problem_id', '')
                problem = next((p for p in problems if p.get('problem_id') == problem_id), {})
                problem_solutions = [s for s in all_solutions if s.get('problem_id') == problem_id]

                # Truncate question to avoid excessive prompt length
                ...
                training_contexts.append({
                    'problem': problem,
                    'solutions': problem_solutions[:4]  # Limit solutions for memory efficiency
                })

            # Add meta-verification training data (Player 4)
            for i, validation in enumerate(all_validation_results[:self.config.problems_per_iteration]):  # Limit for memory
                ...
                training_contexts.append({
                    'problem': problem,
                    'solutions': problem_solutions[:4],  # Limit for memory efficiency
                    'test_cases': problem_tests[:8]  # Limit for memory efficiency
                })

            # Log training data distribution for verification
            role_counts = {}
            for role in training_roles:
                role_counts[role] = role_counts.get(role, 0) + 1
            logger.info(f"GRPO training data: {role_counts}")
    ...
    def _save_checkpoint(self, iteration: int):
        ...
        # Copy checkpoint to Modal volume if it exists (to survive container timeout)
        import shutil
        # Use the actual checkpoint directory name (e.g., phase3_grpo_4player)
        volume_checkpoint_dir = Path("/results") / self.checkpoint_dir.name
        if volume_checkpoint_dir.exists() or volume_checkpoint_dir.parent.exists():
            try:
                volume_checkpoint_dir.mkdir(parents=True, exist_ok=True)

                # Copy checkpoint files
                shutil.copy2(checkpoint_path, volume_checkpoint_dir / f"checkpoint_iteration_{iteration}.json")

                # Copy GRPO checkpoint directory
                volume_grpo_dir = volume_checkpoint_dir / f"grpo_iteration_{iteration}"
                if grpo_checkpoint_dir.exists():
                    shutil.copytree(grpo_checkpoint_dir, volume_grpo_dir, dirs_exist_ok=True)

                # Copy archive
                shutil.copy2(archive_path, volume_checkpoint_dir / f"archive_iteration_{iteration}.json")

                logger.info(f"✅ Checkpoint copied to volume for iteration {iteration}")
            except Exception as e:
                logger.warning(f"Failed to copy checkpoint to volume: {e}")
